...
def support_graph_matrix(labelled_images, labels, unlabeled_images, query_images):
	...
	def intra_graph_connections(a, img_num, img_index):
		...
		for i in range(1,num_neighbors+1):
			# right
			edge_weights.append(alpha*f(a[:, :-i], a[:, i:]) + beta*torch.abs(lab[:, :-i] - lab[:, i:]).reshape(-1))
			...
			edge_weights.append(alpha*f(a[i:, :-i], a[:-i, i:]) + beta*torch.abs(lab[i:, :-i] - lab[:-i, i:]).reshape(-1))
		...
		edges[1].append(indices[2:, 1:].reshape(-1))
		# 1 up and 2 right
		edge_weights.append(alpha*f(a[1:, :-2], a[:-1, 2:]) + beta*torch.abs(lab[1:, :-2] - lab[:-1, 2:]).reshape(-1))
	...
	def inter_graph_connections(a, b, img_num, img_index, prev_img_index):
		...
		edge_weights.append(alpha*f(a - b) + beta*torch.abs(lab - prev_img_lab).reshape(-1))
		...
		edge_weights.append(alpha*f(a[:, :-1], b[:, 1:]) + beta*torch.abs(lab[:, :-1] - prev_img_lab[:, 1:]).reshape(-1))
		...
		edge_weights.append(alpha*f(a[:-1, :-1], b[1:, 1:]) + beta*torch.abs(lab[:-1, :-1] - prev_img_lab[1:, 1:]).reshape(-1))
